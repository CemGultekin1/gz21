...
def dataset_initiator(domain :str = "four_regions"):
    ...
    if domain == "four_regions":
        ...
        # global_ds = global_ds.assign(hu=expanded_hu)
...
# double gyre data Cheng 
def get_length_gyre(varname:str):
    # global_ds =  xr.open_dataset('/scratch/cz3056/CNN_train/Arthur_model/gz21/offline_double_dyre_truth.nc')
    global_ds =  xr.open_zarr('/scratch/cz3056/CNN_train/Arthur_model/double_dyre_forcing.zarr')
    ntimes = len(global_ds.time)
    global train_split,test_split
    train_index = int(train_split * ntimes)
    test_index = int(test_split * ntimes)
...
class LazyDatasetWrapper_gyre(ConcatDataset_):
    def __init__(self, varname,land_mask:str = "None",**_init_kwargs):
        self.varname = varname
        self._lazy_init_flag = False
        self._transform_from_model_flag = False
        ...
        global params
        num_domains = 1
        self._length = get_length_gyre(varname)*num_domains
        self._init_kwargs = _init_kwargs
        self._subset = None
        self._land_mask = land_mask
    def add_transforms_from_model(self,model):
        self._model = model
    def lazy__init__(self,):
        train_dataset,test_dataset,datasets,_,_ = dataset_initiator_gyre(**self._init_kwargs)
        if self.varname == "train_dataset":
            subset =  train_dataset
        else:
            subset =  test_dataset
...
#Cheng
train_dataset_gyre = LazyDatasetWrapper_gyre('train_dataset') #ConcatDataset_(train_datasets)
test_dataset_gyre = LazyDatasetWrapper_gyre('test_dataset') #ConcatDataset_(test_datasets)
test_dataset_for_transform_gyre = LazyDatasetWrapper_gyre('test_dataset')
# FIN DATA---------------------------------------------------------------------
...
net = model_cls(n_in_channels=2,batch_norm = bool(params.batchnorm))#datasets[0].n_features, criterion.n_required_channels) Cheng
try:
...
print('Size of CM2.6 training data: {}'.format(len(train_dataset)))
print('Size of CM2.6 validation data : {}'.format(len(test_dataset)))
# Dataloaders
...
#double gyre Cheng
train_dataset_gyre.add_transforms_from_model(net)
test_dataset_gyre.add_transforms_from_model(net)

print('Size of double gyre training data: {}'.format(len(train_dataset_gyre)))
print('Size of double gyre validation data : {}'.format(len(test_dataset_gyre)))
# Dataloaders
train_dataloader_gyre = DataLoader(train_dataset_gyre, batch_size=batch_size,
                            shuffle=True, drop_last=True, num_workers = 1)#params.num_workers)
test_dataloader_gyre = DataLoader(test_dataset_gyre, batch_size=batch_size,
                            shuffle=False, drop_last=True, num_workers = 1)#params.num_workers)
...
for i_epoch in range(n_epochs):
    ...
    train_loss = trainer.train_for_one_epoch_gyre([train_dataloader,train_dataloader_gyre], optimizer,
                                            lr_scheduler, clip=1.) #Cheng
    test = trainer.test(train_dataloader)#test_dataloader)
    test2 = trainer.test(train_dataloader_gyre)#test_dataloader) #Cheng
    if test == 'EARLY_STOPPING':
        print(test)
        break
        
    test_loss, metrics_results = test
    test_loss_gyre, metrics_results_gyre = test2
    
    # Log the training loss
    print('Train loss for this epoch is ', train_loss)
    print('CM2.6 test loss for this epoch is ', test_loss) #Cheng
    print('Double gyre test loss for this epoch is ', test_loss_gyre) #Cheng
