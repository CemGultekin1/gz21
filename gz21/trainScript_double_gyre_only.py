...
# this does not occur.
train_dataset = LazyDatasetWrapper_gyre('train_dataset') #ConcatDataset_(train_datasets)
test_dataset = LazyDatasetWrapper_gyre('test_dataset') #ConcatDataset_(test_datasets)
test_dataset_for_transform = LazyDatasetWrapper_gyre('test_dataset')
# FIN DATA---------------------------------------------------------------------
...
net = model_cls(n_in_channels=2,batch_norm = bool(params.batchnorm))#datasets[0].n_features, criterion.n_required_channels)
try:
